...
for e in range(epochs):
    ...
    for x, y in zip(features.values, targets):
        ...
        hidden_input = np.dot(x.astype(np.float64), weights_input_hidden.astype(np.float64))
        hidden_output = sigmoid(hidden_input)
        output = sigmoid(np.dot(hidden_output.astype(np.float64), weights_hidden_output.astype(np.float64)))
        ...
        hidden_error = np.dot(output_error_term.astype(np.float64), weights_hidden_output.astype(np.float64))
        ...
        del_w_input_hidden += learnrate * hidden_error_term * x[:, None].astype(np.float64)
